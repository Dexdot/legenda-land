...
import { TweenMax, Power2, Power3, Circ, TweenLite } from 'gsap';
import WEBGL from './WebGL';
import onload from '../section/onload';
...
let stage = {
  ...
  first: function() {
    ...
    TweenLite.to(document.querySelector('.enter-button'), 1, {
      opacity: 1,
      onComplete: () => {
        can2 = true;
      }
  ...
  second: function() {
    ...
    myAnimation = TweenMax.to(camera.position, 3, {
      ease: Power2.easeOut,
      z: -180
    });
      ...
      onComplete: () => {
        ...
        TweenLite.to(camera.position, 5, {
          ...
          onComplete: () => {
            ...
            setTimeout(() => {
              $('.sections').css('pointer-events', 'auto');
              $('.section').addClass('loaded');
              onload(() => {
                $('.towers-canvas').remove();
                cancelAnimationFrame(window.towersRAF);
              });
            }, window.innerWidth > 900 ? 1000 : 2000);
          }
        }); //x-127z-103

        can2 = false;

        TweenMax.to(blackplane.position, 3, {
          ease: Power2.easeOut,
          x: blackplane.position.x - 20,
          z: blackplane.position.z - 20
        });

        TweenMax.to(blackplane2.position, 3, {
          ease: Power2.easeOut,
          x: blackplane2.position.x - 20,
          z: blackplane2.position.z - 20
        });
        ...
        for (let i = 0; i < GroupArray.length; i++) {
          ...
          TweenMax.to(GroupArray[i].position, 3, {
            ease: Power2.easeOut,
            x: GroupArray[i].position.x - 20,
            z: GroupArray[i].position.z - 20
          });
        }
...
function Init() {
  ...
  renderer.domElement.classList.add('towers-canvas');
  document.querySelector('main').appendChild(renderer.domElement); //помещение отрендеренного канваса в DOM
  ...
  light.position.set(-87, 180, -20);

  blackgeom = new THREE.PlaneGeometry(70, 500, 1);
  blackmat = new THREE.MeshBasicMaterial({
    color: 0x231f20,
    side: THREE.DoubleSide
  });
...
function animate() {
  window.towersRAF = requestAnimationFrame(animate);
  composer.render();
...
const headshot = () => {
  if (can2) {
    for (let i = 0; i < GroupArray2.length; i++) {
      ...
      GroupArray2[i].rotanim = TweenMax.to(GroupArray2[i].rotation, 3, {
        ease: Power3.easeOut,
        y: GroupArray2[i].mustrotate,
        onComplete: () => {
          GroupArray2[i].moving = true;
        }
      });
...
document.querySelector('.js-webgl').addEventListener('click', headshot);
